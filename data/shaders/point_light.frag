...
void main() {
    ...
    vec3 l = -normalize(position.xyz - u_light_position);
    vec3 h = normalize(v + l);

	float light_distance = length(position.xyz - u_light_position);
	float attenuation = pow(clamp(1.0 - pow(light_distance / u_light_radius, 4.0), 0.0, 1.0), 2.0) / (light_distance * light_distance + 1.0);
    ...
    vec3 lo = (kd * albedo / PI + specular) * radiance * intensity * n_dot_l;
