...
namespace rb {
	...
	public:
		...
		static std::vector<std::uint8_t> compress(const span<const T>& uncompressed_data) {
			...
			const auto compressed_size = compress(uncompressed_data.data(), uncompressed_data.size_bytes(), compressed_data.data());
		...
		static std::vector<T> uncompress(std::size_t uncompressed_size, const span<const std::uint8_t>& compressed_data) {
			...
			const auto size = uncompress(compressed_data.data(), compressed_data.size_bytes(), uncompressed_data.data());
