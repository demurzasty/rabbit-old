...
namespace rb {
    ...
    template<typename T>
    constexpr vec3<T> transform_normal(const mat4<T>& mat, const vec3<T>& vec) {
        return {
    ...
    template<typename T>
    constexpr mat4<T> inverse(const mat4<T>& mat) {
        const mat4<T> out = { {
            mat[5] * mat[10] * mat[15] -
            mat[5] * mat[11] * mat[14] -
            mat[9] * mat[6] * mat[15] +
    ...
    template<typename T>  
    constexpr vec3<T> unproject(const vec3<T>& vec, const vec4<T>& viewport, const mat4<T>& inv_proj_view) {
