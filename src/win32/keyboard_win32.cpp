...
void keyboard_win32::refresh() {
    std::memcpy(_last_state, _state, sizeof(_state));
    GetKeyboardState(_state);
}

bool keyboard_win32::is_key_down(keycode key) {
    const auto index = key_map[key];
    return _state[index] & 0x80;
}

bool keyboard_win32::is_key_up(keycode key) {
    const auto index = key_map[key];
    return (_state[index] & 0x80) == 0;
}

bool keyboard_win32::is_key_pressed(keycode key) {
    const auto index = key_map[key];
    return (_state[index] & 0x80) && ((_last_state[index] & 0x80) == 0);
}

bool keyboard_win32::is_key_released(keycode key) {
    const auto index = key_map[key];
    return ((_state[index] & 0x80) == 0) && (_last_state[index] & 0x80);
}
